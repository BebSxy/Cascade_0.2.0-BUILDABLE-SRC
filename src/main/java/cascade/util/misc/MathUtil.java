...
implements Util {
    ...
    public static double getRandom(double min, double max) {
        return MathHelper.clamp_double((double)(min + random.nextDouble() * max), (double)min, (double)max);
    }

    public static float getRandom(float min, float max) {
        return MathHelper.clamp_float((float)(min + random.nextFloat() * max), (float)min, (float)max);
    }
    ...
    public static double[] directionSpeed(double speed) {
        float forward = MathUtil.mc.thePlayer.movementInput.moveForward;
        float side = MathUtil.mc.thePlayer.movementInput.moveStrafe;
        float yaw = MathUtil.mc.thePlayer.prevRotationYaw + (MathUtil.mc.thePlayer.rotationYaw - MathUtil.mc.thePlayer.prevRotationYaw) * mc.getRenderPartialTicks();
        if (forward != 0.0f) {
            if (side > 0.0f) {
    ...
    public static float[] calcAngle(Vec3d from, Vec3d to) {
        ...
        double dist = MathHelper.sqrt_double((double)(difX * difX + difZ * difZ));
        return new float[]{(float)MathHelper.wrapAngleTo180_double((double)(Math.toDegrees(Math.atan2(difZ, difX)) - 90.0)), (float)MathHelper.wrapAngleTo180_double((double)Math.toDegrees(Math.atan2(difY, dist)))};
    }
    ...
    public static RayTraceResult rayTraceBlocks(Vec3d vec31, Vec3d vec32, boolean stopOnLiquid, boolean ignoreNoBox, boolean returnLastUncollidableBlock, boolean ignoreWebs) {
        ...
        if (!(Double.isNaN(vec32.xCoord) || Double.isNaN(vec32.yCoord) || Double.isNaN(vec32.zCoord))) {
            ...
            IBlockState state = MathUtil.mc.theWorld.getBlockState(pos);
            Block block = state.getBlock();
            if (!(ignoreNoBox && state.getCollisionBoundingBox((IBlockAccess)MathUtil.mc.theWorld, pos) == Block.NULL_AABB || !block.canCollideCheck(state, stopOnLiquid) || ignoreWebs && block instanceof BlockWeb || (raytraceresult = state.collisionRayTrace((World)MathUtil.mc.theWorld, pos, vec31, vec32)) == null)) {
            ...
            while (k1-- >= 0) {
                ...
                IBlockState state2 = MathUtil.mc.theWorld.getBlockState(pos);
                Block block2 = state2.getBlock();
                if (ignoreNoBox && state2.getMaterial() != Material.portal && state2.getCollisionBoundingBox((IBlockAccess)MathUtil.mc.theWorld, pos) == Block.NULL_AABB) continue;
                if (!(!block2.canCollideCheck(state2, stopOnLiquid) || ignoreWebs && block2 instanceof BlockWeb)) {
                    RayTraceResult raytraceresult3 = state2.collisionRayTrace((World)MathUtil.mc.theWorld, pos, vec31, vec32);
    ...
    public static EntityPlayer placeValue(double x, double y, double z, EntityPlayer entity) {
        List list1 = MathUtil.mc.theWorld.getCollisionBoxes((Entity)entity, entity.getEntityBoundingBox().addCoord(x, y, z));
        if (y != 0.0) {
            int l = list1.size();
            for (int k = 0; k < l; ++k) {
