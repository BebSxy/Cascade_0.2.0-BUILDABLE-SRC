...
implements Util {
    public static void swingArm(SwingType swingType) {
        switch (swingType) {
            case MainHand: {
                EntityUtil.mc.thePlayer.swingArm(EnumHand.MAIN_HAND);
                break;
            }
            case OffHand: {
                EntityUtil.mc.thePlayer.swingArm(EnumHand.OFF_HAND);
                break;
            }
            case Packet: {
                mc.getNetHandler().addToSendQueue((Packet)new CPacketAnimation(EntityUtil.mc.thePlayer.getHeldItemMainhand().getItem().equals(Items.END_CRYSTAL) ? EnumHand.MAIN_HAND : EnumHand.OFF_HAND));
            }
        }
    }
    ...
    public static float calculatePosDamage(double posX, double posY, double posZ, Entity entity) {
        ...
        double blockDensity = entity.worldObj.getBlockDensity(vec3d, entity.getEntityBoundingBox());
    ...
    public static RayTraceResult rayTraceBlocks(Vec3d vec31, Vec3d vec32, boolean stopOnLiquid, boolean ignoreBlockWithoutBoundingBox, boolean returnLastUncollidableBlock) {
        ...
        IBlockState iblockstate = EntityUtil.mc.theWorld.getBlockState(blockpos);
        Block block = iblockstate.getBlock();
        if ((!ignoreBlockWithoutBoundingBox || iblockstate.getCollisionBoundingBox((IBlockAccess)EntityUtil.mc.theWorld, blockpos) != Block.NULL_AABB) && block.canCollideCheck(iblockstate, stopOnLiquid)) {
            return iblockstate.collisionRayTrace((World)EntityUtil.mc.theWorld, blockpos, vec31, vec32);
        }
        ...
        while (k1-- >= 0) {
            ...
            IBlockState iblockstate1 = EntityUtil.mc.theWorld.getBlockState(blockpos);
            Block block1 = iblockstate1.getBlock();
            if (ignoreBlockWithoutBoundingBox && iblockstate1.getMaterial() != Material.portal && iblockstate1.getCollisionBoundingBox((IBlockAccess)EntityUtil.mc.theWorld, blockpos) == Block.NULL_AABB) continue;
            if (block1.canCollideCheck(iblockstate1, stopOnLiquid) && block1 != Blocks.web) {
                return iblockstate1.collisionRayTrace((World)EntityUtil.mc.theWorld, blockpos, vec31, vec32);
            }
    ...
    private static float getMultipliedDamage(float damage) {
        return damage * (EntityUtil.mc.theWorld.getDifficulty().getDifficultyId() == 0 ? 0.0f : (EntityUtil.mc.theWorld.getDifficulty().getDifficultyId() == 2 ? 1.0f : (EntityUtil.mc.theWorld.getDifficulty().getDifficultyId() == 1 ? 0.5f : 1.5f)));
    }
    ...
    public static List<Vec3d> getUnsafeBlocksFromVec3d(Vec3d pos, int height, boolean floor) {
        ...
        for (Vec3d vector : EntityUtil.getOffsets(height, floor)) {
            ...
            Block block = EntityUtil.mc.theWorld.getBlockState(targetPos).getBlock();
    ...
    public static boolean isObbyHole(BlockPos blockPos) {
        ...
        for (BlockPos pos : touchingBlocks) {
            IBlockState touchingState = EntityUtil.mc.theWorld.getBlockState(pos);
    ...
    public static boolean isMoving() {
        return EntityUtil.mc.thePlayer.moveForward != 0.0f || EntityUtil.mc.thePlayer.moveStrafing != 0.0f;
    }
    ...
    public static boolean isPlayerSafe(EntityPlayer target) {
        ...
        return !(EntityUtil.mc.theWorld.getBlockState(playerPos.down()).getBlock() != Blocks.obsidian && EntityUtil.mc.theWorld.getBlockState(playerPos.down()).getBlock() != Blocks.bedrock || EntityUtil.mc.theWorld.getBlockState(playerPos.north()).getBlock() != Blocks.obsidian && EntityUtil.mc.theWorld.getBlockState(playerPos.north()).getBlock() != Blocks.bedrock || EntityUtil.mc.theWorld.getBlockState(playerPos.east()).getBlock() != Blocks.obsidian && EntityUtil.mc.theWorld.getBlockState(playerPos.east()).getBlock() != Blocks.bedrock || EntityUtil.mc.theWorld.getBlockState(playerPos.south()).getBlock() != Blocks.obsidian && EntityUtil.mc.theWorld.getBlockState(playerPos.south()).getBlock() != Blocks.bedrock || EntityUtil.mc.theWorld.getBlockState(playerPos.west()).getBlock() != Blocks.obsidian && EntityUtil.mc.theWorld.getBlockState(playerPos.west()).getBlock() != Blocks.bedrock);
    }
    ...
    public static double getMaxSpeed() {
        ...
        if (EntityUtil.mc.thePlayer != null && EntityUtil.mc.thePlayer.isPotionActive(Objects.requireNonNull(Potion.getPotionById((int)1)))) {
            maxModifier *= 1.0 + 0.2 * (double)(Objects.requireNonNull(EntityUtil.mc.thePlayer.getActivePotionEffect(Objects.requireNonNull(Potion.getPotionById((int)1)))).getAmplifier() + 1);
        }
        return maxModifier;
    }
    ...
    public static boolean isInLiquid() {
        return EntityUtil.mc.thePlayer.isInWater() || EntityUtil.mc.thePlayer.isInLava();
    }

    public static double[] forward(double speed) {
        float forward = EntityUtil.mc.thePlayer.movementInput.moveForward;
        float side = EntityUtil.mc.thePlayer.movementInput.moveStrafe;
        float yaw = EntityUtil.mc.thePlayer.prevRotationYaw + (EntityUtil.mc.thePlayer.rotationYaw - EntityUtil.mc.thePlayer.prevRotationYaw) * mc.getRenderPartialTicks();
        if (forward != 0.0f) {
            if (side > 0.0f) {
    ...
    public static boolean isBurrow(Entity entity) {
        ...
        return EntityUtil.mc.theWorld.getBlockState(blockPos).getBlock() == Blocks.obsidian || EntityUtil.mc.theWorld.getBlockState(blockPos).getBlock() == Blocks.ender_chest;
    }
    ...
    public static BlockPos getPlayerPosWithEntity() {
        return new BlockPos(EntityUtil.mc.thePlayer.getRidingEntity() != null ? EntityUtil.mc.thePlayer.getRidingEntity().posX : EntityUtil.mc.thePlayer.posX, EntityUtil.mc.thePlayer.getRidingEntity() != null ? EntityUtil.mc.thePlayer.getRidingEntity().posY : EntityUtil.mc.thePlayer.posY, EntityUtil.mc.thePlayer.getRidingEntity() != null ? EntityUtil.mc.thePlayer.getRidingEntity().posZ : EntityUtil.mc.thePlayer.posZ);
    }
    ...
    public static boolean isInWater(Entity entity) {
        ...
        for (int x = MathHelper.floor_double((double)entity.posX); x < MathHelper.ceiling_double_int((double)entity.posX); ++x) {
            for (int z = MathHelper.floor_double((double)entity.posZ); z < MathHelper.ceiling_double_int((double)entity.posZ); ++z) {
                ...
                if (!(EntityUtil.mc.theWorld.getBlockState(pos).getBlock() instanceof BlockLiquid)) continue;
                return true;
            }
        }
        return false;
    ...
    public static EntityPlayer getClosestEnemy(List<EntityPlayer> list) {
        return EntityUtil.getClosestEnemy(EntityUtil.mc.thePlayer.getPositionVector(), list);
    }
    ...
    public static boolean isOnChest(Entity entity) {
        BlockPos blockPos = new BlockPos(entity.posX, entity.posY, entity.posZ);
        return EntityUtil.mc.theWorld.getBlockState(blockPos).getBlock().equals(Blocks.obsidian) || EntityUtil.mc.theWorld.getBlockState(blockPos).getBlock().equals(Blocks.ender_chest);
    }

    public static boolean isOnGround(double x, double y, double z, Entity entity) {
        try {
            double d3 = y;
            List list1 = EntityUtil.mc.theWorld.getCollisionBoxes(entity, entity.getEntityBoundingBox().addCoord(x, y, z));
            if (y != 0.0) {
                int l = list1.size();
                for (int k = 0; k < l; ++k) {
