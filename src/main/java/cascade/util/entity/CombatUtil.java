...
implements Util {
    ...
    public static void attackEntity(Entity entity, boolean packet, boolean swingArm) {
        ...
        if (swingArm) {
            CombatUtil.mc.thePlayer.swingArm(EnumHand.MAIN_HAND);
        }
    }
    ...
    public static float getBlastReduction(EntityLivingBase entity, float damageI) {
        ...
        damage *= 1.0f - MathHelper.clamp_float((float)EnchantmentHelper.getEnchantmentModifierDamage((Iterable)entity.getArmorInventoryList(), (DamageSource)EXPLOSION_SOURCE), (float)0.0f, (float)20.0f) / 25.0f;
    ...
    public static float getDamageMultiplied(float damage) {
        int diff = CombatUtil.mc.theWorld.getDifficulty().getDifficultyId();
    ...
    public static RayTraceResult rayTraceBlocks(Vec3d vec31, Vec3d vec32, boolean stopOnLiquid, boolean ignoreBlockWithoutBoundingBox, boolean returnLastUncollidableBlock) {
        ...
        IBlockState iblockstate = CombatUtil.mc.theWorld.getBlockState(blockpos);
        Block block = iblockstate.getBlock();
        if ((!ignoreBlockWithoutBoundingBox || iblockstate.getCollisionBoundingBox((IBlockAccess)CombatUtil.mc.theWorld, blockpos) != Block.NULL_AABB) && block.canCollideCheck(iblockstate, stopOnLiquid)) {
            return iblockstate.collisionRayTrace((World)CombatUtil.mc.theWorld, blockpos, vec31, vec32);
        }
        ...
        while (k2-- >= 0) {
            ...
            IBlockState iblockstate2 = CombatUtil.mc.theWorld.getBlockState(blockpos);
            Block block2 = iblockstate2.getBlock();
            if (ignoreBlockWithoutBoundingBox && iblockstate2.getMaterial() != Material.portal && iblockstate2.getCollisionBoundingBox((IBlockAccess)CombatUtil.mc.theWorld, blockpos) == Block.NULL_AABB) continue;
            if (block2.canCollideCheck(iblockstate2, stopOnLiquid) && !(block2 instanceof BlockWeb)) {
                return iblockstate2.collisionRayTrace((World)CombatUtil.mc.theWorld, blockpos, vec31, vec32);
            }
