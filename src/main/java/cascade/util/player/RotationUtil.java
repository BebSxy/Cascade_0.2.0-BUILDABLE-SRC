...
implements Util {
    ...
    public static void faceVector(Vec3d vec, boolean normalizeAngle) {
        ...
        RotationUtil.mc.thePlayer.sendQueue.addToSendQueue((Packet)new CPacketPlayer.Rotation(rotations[0], normalizeAngle ? (float)MathHelper.normalizeAngle((int)((int)rotations[1]), (int)360) : rotations[1], RotationUtil.mc.thePlayer.onGround));
    }

    public static void faceEntity(Entity entity) {
        float[] angle = MathUtil.calcAngle(RotationUtil.mc.thePlayer.getPositionEyes(mc.getRenderPartialTicks()), entity.getPositionEyes(mc.getRenderPartialTicks()));
        RotationUtil.faceYawAndPitch(angle[0], angle[1]);
    }

    public static float[] getAngle(Entity entity) {
        return MathUtil.calcAngle(RotationUtil.mc.thePlayer.getPositionEyes(mc.getRenderPartialTicks()), entity.getPositionEyes(mc.getRenderPartialTicks()));
    }
    ...
    public static boolean isInFov(BlockPos pos) {
        return pos != null && (RotationUtil.mc.thePlayer.getDistanceSq(pos) < 4.0 || RotationUtil.yawDist(pos) < (double)(RotationUtil.getHalvedfov() + 2.0f));
    }
    ...
    public static double yawDist(BlockPos pos) {
        if (pos != null) {
            Vec3d difference = new Vec3d((Vec3i)pos).subtract(RotationUtil.mc.thePlayer.getPositionEyes(mc.getRenderPartialTicks()));
    ...
    public static double yawDist(Entity e) {
        if (e != null) {
            Vec3d difference = e.getPositionVector().addVector(0.0, (double)(e.getEyeHeight() / 2.0f), 0.0).subtract(RotationUtil.mc.thePlayer.getPositionEyes(mc.getRenderPartialTicks()));
            double d = Math.abs((double)RotationUtil.mc.thePlayer.rotationYaw - (Math.toDegrees(Math.atan2(difference.zCoord, difference.xCoord)) - 90.0)) % 360.0;
            return d > 180.0 ? 360.0 - d : d;
        }
        return 0.0;
    ...
    public static HoleUtil.Hole getTargetHoleVec3D(double targetRange) {
        return HoleUtil.getHoles(targetRange, RotationUtil.getPlayerPos(), false).stream().filter(hole -> RotationUtil.mc.thePlayer.getPositionVector().distanceTo(new Vec3d((double)hole.pos1.getX() + 0.5, RotationUtil.mc.thePlayer.posY, (double)hole.pos1.getZ() + 0.5)) <= targetRange).min(Comparator.comparingDouble(hole -> RotationUtil.mc.thePlayer.getPositionVector().distanceTo(new Vec3d((double)hole.pos1.getX() + 0.5, RotationUtil.mc.thePlayer.posY, (double)hole.pos1.getZ() + 0.5)))).orElse(null);
    }
