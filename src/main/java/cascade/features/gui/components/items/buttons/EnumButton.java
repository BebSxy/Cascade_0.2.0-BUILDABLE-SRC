    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        if (this.setting.isOpen) {
            for (Object obber : this.setting.getValue().getClass().getEnumConstants()) {
                ...
                EnumButton.mc.fontRendererObj.drawStringWithShadow((this.setting.getValueAsString().equals(obber.toString()) ? ChatFormatting.WHITE : ChatFormatting.GRAY) + obber.toString(), this.x + 4.0f, (float)y + (float)this.height / 2.0f - (float)EnumButton.mc.fontRendererObj.FONT_HEIGHT / 2.0f, -1);
            }
        ...
        switch (ClickGui.getInstance().moduleSuffix.getValue()) {
            ...
            case CustomText: {
                EnumButton.mc.fontRendererObj.drawStringWithShadow(this.setting.isOpen ? ClickGui.getInstance().opened.getValue() : ClickGui.getInstance().closed.getValue(), this.x + (float)this.width - (float)EnumButton.mc.fontRendererObj.getStringWidth(this.setting.isOpen ? ClickGui.getInstance().opened.getValue() : ClickGui.getInstance().closed.getValue()) + 5.0f, this.y + (float)this.height / 2.0f - (float)EnumButton.mc.fontRendererObj.FONT_HEIGHT / 2.0f + 1.0f, -1);
                break;
            }
            case New: {
