...
extends Module {
    ...
    public void onPacketReceive(PacketEvent.Receive e) {
        ...
        if (e.getStage() == 0 && Velocity.mc.thePlayer != null) {
            ...
            if (this.knockBack.getValue().booleanValue() && e.getPacket() instanceof SPacketEntityVelocity && (velocity = (SPacketEntityVelocity)e.getPacket()).getEntityID() == Velocity.mc.thePlayer.entityId) {
                e.setCanceled(true);
            }
            if (e.getPacket() instanceof SPacketEntityStatus && this.bobbers.getValue().booleanValue() && (packet = (SPacketEntityStatus)e.getPacket()).getOpCode() == 31 && (entity = packet.getEntity((World)Velocity.mc.theWorld)) instanceof EntityFishHook) {
