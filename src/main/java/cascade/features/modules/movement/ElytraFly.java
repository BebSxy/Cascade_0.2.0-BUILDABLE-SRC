...
extends Module {
    ...
    void onMove(MoveEvent e) {
        ...
        ItemStack stack = ElytraFly.mc.thePlayer.getItemStackFromSlot(EntityEquipmentSlot.CHEST);
        if (!PlayerUtil.isElytraEquipped() || !ItemElytra.isUsable((ItemStack)stack)) {
            return;
        }
        if (ElytraFly.mc.thePlayer.isElytraFlying() && (this.noLiquid.getValue().booleanValue() && EntityUtil.isInLiquid() || this.noGround.getValue().booleanValue() && ElytraFly.mc.thePlayer.onGround)) {
        ...
        if (this.autoStart.getValue().booleanValue() && ElytraFly.mc.gameSettings.keyBindJump.getIsKeyPressed() && !ElytraFly.mc.thePlayer.isElytraFlying() && ElytraFly.mc.thePlayer.motionY < 0.0) {
            if (this.timerMode.getValue() != TimerMode.None) {
            ...
            if (this.infDura.getValue().booleanValue() && ElytraFly.mc.thePlayer.isElytraFlying()) {
                mc.getNetHandler().addToSendQueue((Packet)new CPacketEntityAction((Entity)ElytraFly.mc.thePlayer, CPacketEntityAction.Action.START_FALL_FLYING));
            }
        }
        switch (this.mode.getValue()) {
            case Control: {
                if (!ElytraFly.mc.thePlayer.isElytraFlying()) break;
                if (!ElytraFly.mc.thePlayer.movementInput.forwardKeyDown && !ElytraFly.mc.thePlayer.movementInput.sneak) {
                    ElytraFly.mc.thePlayer.setVelocity(0.0, 0.0, 0.0);
                    break;
                }
                if (!ElytraFly.mc.thePlayer.movementInput.forwardKeyDown) break;
            ...
            case Boost: {
                if (!ElytraFly.mc.thePlayer.movementInput.jump || !ElytraFly.mc.thePlayer.isElytraFlying()) break;
