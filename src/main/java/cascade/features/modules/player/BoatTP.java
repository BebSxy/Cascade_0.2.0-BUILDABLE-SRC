...
extends Module {
    ...
    public void onEnable() {
        if (BoatTP.fullNullCheck() || !BoatTP.mc.thePlayer.isRiding()) {
            return;
        }
    ...
    public void onUpdate() {
        ...
        if (BoatTP.mc.thePlayer.getRidingEntity() != null && BoatTP.mc.thePlayer.getRidingEntity() instanceof EntityBoat) {
            this.boat = (EntityBoat)BoatTP.mc.thePlayer.getRidingEntity();
        }
        if (this.boat == null) {
    ...
    public int findNextAvailableSpaceUp(int start) {
        ...
        for (int i = start + 1; i < (BoatTP.mc.thePlayer.dimension != 1 ? 257 : 125); ++i) {
            if (!BoatTP.mc.theWorld.getBlockState((BlockPos)new BlockPos((int)playerX, (int)i, (int)playerZ)).getBlock().blockMaterial.equals(Material.air) || !BoatTP.mc.theWorld.getBlockState((BlockPos)new BlockPos((int)playerX, (int)(i + 1), (int)playerZ)).getBlock().blockMaterial.equals(Material.air) || BoatTP.mc.theWorld.getBlockState((BlockPos)new BlockPos((int)playerX, (int)(i - 1), (int)playerZ)).getBlock().blockMaterial.equals(Material.air)) continue;
            up = i;
            return up;
        }
        return up;
