...
extends Module {
    ...
    public void onUpdate() {
        ...
        if (this.currentPos != null) {
            if (Mine.mc.thePlayer.getDistanceSq(this.currentPos) > MathUtil.square(this.range.getValue().floatValue())) {
                this.currentPos = null;
                this.blockState = null;
                return;
            }
            if (Mine.mc.theWorld.getBlockState(this.currentPos) != this.blockState || Mine.mc.theWorld.getBlockState(this.currentPos).getBlock() == Blocks.air) {
                this.currentPos = null;
                this.blockState = null;
            }
        }
    ...
    public void onPacketSend(PacketEvent.Send e) {
        ...
        if (e.getPacket() instanceof CPacketPlayerDigging && (packet = (CPacketPlayerDigging)e.getPacket()) != null && packet.getPosition() != null) {
            try {
                for (Entity entity : Mine.mc.theWorld.getEntitiesWithinAABBExcludingEntity(null, new AxisAlignedBB(packet.getPosition()))) {
    ...
    public void onBlockBreak(BlockEvent e) {
        ...
        if (e.getStage() == 4 && BlockUtil.canBreak(e.pos)) {
            if (this.currentPos == null) {
                ...
                this.blockState = Mine.mc.theWorld.getBlockState(this.currentPos);
                this.timer.reset();
            }
            Mine.mc.thePlayer.swingArm(EnumHand.MAIN_HAND);
