...
extends Module {
    ...
    public void onClickBlock(BlockEvent event) {
        Block block;
        if (event.getStage() == 3 && this.mode.getValue() != Mode.All && (block = Nuker.mc.theWorld.getBlockState(event.pos).getBlock()) != null && block != this.selected) {
            this.selected = block;
                ...
                if (pos != null) {
                    if (this.mode.getValue() != Mode.Nuke) {
                        if (this.rotate.getValue().booleanValue()) {
                            float[] angle = MathUtil.calcAngle(Nuker.mc.thePlayer.getPositionEyes(mc.getRenderPartialTicks()), new Vec3d((double)((float)pos.getX() + 0.5f), (double)((float)pos.getY() + 0.5f), (double)((float)pos.getZ() + 0.5f)));
                            Cascade.rotationManager.setPlayerRotations(angle[0], angle[1]);
                        }
                        if (this.canBreak(pos)) {
                            Nuker.mc.playerController.onPlayerDamageBlock(pos, Nuker.mc.thePlayer.getHorizontalFacing());
                            Nuker.mc.thePlayer.swingArm(EnumHand.MAIN_HAND);
                        }
                    } else {
                        for (int i = 0; i < this.blockPerTick.getValue(); ++i) {
                            ...
                            if (this.rotate.getValue().booleanValue()) {
                                float[] angle2 = MathUtil.calcAngle(Nuker.mc.thePlayer.getPositionEyes(mc.getRenderPartialTicks()), new Vec3d((double)((float)pos.getX() + 0.5f), (double)((float)pos.getY() + 0.5f), (double)((float)pos.getZ() + 0.5f)));
    ...
    private boolean canBreak(BlockPos pos) {
        IBlockState blockState = Nuker.mc.theWorld.getBlockState(pos);
        Block block = blockState.getBlock();
        return block.getBlockHardness(blockState, (World)Nuker.mc.theWorld, pos) != -1.0f;
    }
        ...
        for (double x = maxDist; x >= -maxDist; x -= 1.0) {
            for (double y = maxDist; y >= -maxDist; y -= 1.0) {
                for (double z = maxDist; z >= -maxDist; z -= 1.0) {
                    ...
                    double dist = Nuker.mc.thePlayer.getDistanceSq((double)pos.getX(), (double)pos.getY(), (double)pos.getZ());
                    if (!(dist <= maxDist) || !blocks.contains(Nuker.mc.theWorld.getBlockState(pos).getBlock()) || !this.canBreak(pos)) continue;
                    maxDist = dist;
                    ret = pos;
                }
            }
        ...
        for (float x = maxDist; x >= -maxDist; x -= 1.0f) {
            for (float y = maxDist; y >= -maxDist; y -= 1.0f) {
                for (float z = maxDist; z >= -maxDist; z -= 1.0f) {
                    ...
                    double dist = Nuker.mc.thePlayer.getDistance((double)pos.getX(), (double)pos.getY(), (double)pos.getZ());
                    if (!(dist <= (double)maxDist) || Nuker.mc.theWorld.getBlockState(pos).getBlock() == Blocks.air || Nuker.mc.theWorld.getBlockState(pos).getBlock() instanceof BlockLiquid || !this.canBreak(pos) || !((double)pos.getY() >= Nuker.mc.thePlayer.posY)) continue;
                    maxDist = (float)dist;
                    ret = pos;
                }
            }
        ...
        for (float x = maxDist; x >= -maxDist; x -= 1.0f) {
            for (float y = maxDist; y >= -maxDist; y -= 1.0f) {
                for (float z = maxDist; z >= -maxDist; z -= 1.0f) {
                    BlockPos pos = new BlockPos(Nuker.mc.thePlayer.posX + (double)x, Nuker.mc.thePlayer.posY + (double)y, Nuker.mc.thePlayer.posZ + (double)z);
                    double dist = Nuker.mc.thePlayer.getDistance((double)pos.getX(), (double)pos.getY(), (double)pos.getZ());
                    if (!(dist <= (double)maxDist) || Nuker.mc.theWorld.getBlockState(pos).getBlock() == Blocks.air || Nuker.mc.theWorld.getBlockState(pos).getBlock() instanceof BlockLiquid || Nuker.mc.theWorld.getBlockState(pos).getBlock() != this.selected || !this.canBreak(pos) || !((double)pos.getY() >= Nuker.mc.thePlayer.posY)) continue;
                    maxDist = (float)dist;
                    ret = pos;
                }
            }
