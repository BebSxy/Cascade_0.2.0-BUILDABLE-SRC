...
extends Module {
    ...
    public void onEnable() {
        if (!Freecam.fullNullCheck()) {
            this.oldBoundingBox = Freecam.mc.thePlayer.getEntityBoundingBox();
            Freecam.mc.thePlayer.setEntityBoundingBox(new AxisAlignedBB(Freecam.mc.thePlayer.posX, Freecam.mc.thePlayer.posY, Freecam.mc.thePlayer.posZ, Freecam.mc.thePlayer.posX, Freecam.mc.thePlayer.posY, Freecam.mc.thePlayer.posZ));
            if (Freecam.mc.thePlayer.getRidingEntity() != null) {
                this.riding = Freecam.mc.thePlayer.getRidingEntity();
                Freecam.mc.thePlayer.dismountRidingEntity();
            }
            ...
            this.position = Freecam.mc.thePlayer.getPositionVector();
    ...
    public void onDisable() {
        if (!Freecam.fullNullCheck()) {
            Freecam.mc.thePlayer.setEntityBoundingBox(this.oldBoundingBox);
            if (this.riding != null) {
                Freecam.mc.thePlayer.startRiding(this.riding, true);
            }
            if (entity != null) {
    ...
    public void onUpdate() {
        ...
        if (this.view.getValue().booleanValue() && !Freecam.mc.gameSettings.keyBindSneak.getIsKeyPressed() && !Freecam.mc.gameSettings.keyBindJump.getIsKeyPressed()) {
            Freecam.mc.thePlayer.motionY = (double)this.speed.getValue().floatValue() * -MathUtil.degToRad(Freecam.mc.thePlayer.rotationPitch) * (double)Freecam.mc.thePlayer.movementInput.moveForward;
        }
    ...
    public void onPacketReceive(PacketEvent.Receive e) {
        ...
        if (e.getPacket() instanceof SPacketSetPassengers && (riding = Freecam.mc.theWorld.getEntityByID(((SPacketSetPassengers)e.getPacket()).getEntityId())) != null && riding == this.riding) {
            this.riding = null;
        }
        if (e.getPacket() instanceof SPacketPlayerPosLook) {
            ...
            if (this.packet.getValue().booleanValue()) {
                ...
                Freecam.mc.thePlayer.sendQueue.addToSendQueue((Packet)new CPacketConfirmTeleport(p.getTeleportId()));
                e.setCanceled(true);
            } else {
                e.setCanceled(true);
            }
