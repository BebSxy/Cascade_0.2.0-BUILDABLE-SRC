...
extends Module {
    ...
    void place(BlockPos posI, EnumFacing face) {
        ...
        if (!Scaffold.mc.thePlayer.isSneaking() && BlockUtil.blackList.contains(block = Scaffold.mc.theWorld.getBlockState(pos).getBlock())) {
        ...
        if (!(Scaffold.mc.thePlayer.getHeldItemMainhand().getItem() instanceof ItemBlock)) {
        ...
        if (this.rotate.getValue().booleanValue()) {
            float[] angle = MathUtil.calcAngle(Scaffold.mc.thePlayer.getPositionEyes(mc.getRenderPartialTicks()), new Vec3d((double)((float)pos.getX() + 0.5f), (double)((float)pos.getY() - 0.5f), (double)((float)pos.getZ() + 0.5f)));
            mc.getNetHandler().addToSendQueue((Packet)new CPacketPlayer.Rotation(angle[0], (float)MathHelper.normalizeAngle((int)((int)angle[1]), (int)360), Scaffold.mc.thePlayer.onGround));
        }
        Scaffold.mc.playerController.processRightClickBlock(Scaffold.mc.thePlayer, Scaffold.mc.theWorld, pos, face, new Vec3d(0.5, 0.5, 0.5), EnumHand.MAIN_HAND);
        Scaffold.mc.thePlayer.swingArm(EnumHand.MAIN_HAND);
