...
extends Module {
    ...
    public void onUpdate() {
        ...
        if (this.autoDuelOn) {
            ...
            if (this.autoDuelOn) {
                if (!this.obbySlotDone && !XCarry.mc.thePlayer.inventory.getStackInSlot((int)(this.obbySlot.getValue().intValue() - 1)).isEmpty) {
                    this.addTasks(36 + this.obbySlot.getValue() - 1);
                }
                this.obbySlotDone = true;
                if (!this.slot1done && !((Slot)XCarry.mc.thePlayer.inventoryContainer.inventorySlots.get((int)this.slot1.getValue().intValue())).getStack().isEmpty) {
                    this.addTasks(this.slot1.getValue());
                }
                this.slot1done = true;
                if (!this.slot2done && !((Slot)XCarry.mc.thePlayer.inventoryContainer.inventorySlots.get((int)this.slot2.getValue().intValue())).getStack().isEmpty) {
                    this.addTasks(this.slot2.getValue());
                }
                this.slot2done = true;
                if (!this.slot3done && !((Slot)XCarry.mc.thePlayer.inventoryContainer.inventorySlots.get((int)this.slot3.getValue().intValue())).getStack().isEmpty) {
    ...
    public void onDisable() {
        if (!XCarry.fullNullCheck()) {
            if (!this.simpleMode.getValue().booleanValue()) {
                ...
                mc.getNetHandler().addToSendQueue((Packet)new CPacketCloseWindow(XCarry.mc.thePlayer.inventoryContainer.windowId));
            }
        }
    }
    ...
    public void onCloseGuiScreen(PacketEvent.Send event) {
        if (this.simpleMode.getValue().booleanValue() && event.getPacket() instanceof CPacketCloseWindow) {
            ...
            if (packet.windowId == XCarry.mc.thePlayer.inventoryContainer.windowId) {
    ...
    private void closeGui() {
        if (this.guiNeedsClose.compareAndSet(true, false) && !XCarry.fullNullCheck()) {
            ...
            XCarry.mc.thePlayer.closeScreen();
    ...
    extends GuiInventory {
        GuiInventoryWrapper() {
            super((EntityPlayer)Util.mc.thePlayer);
        }
