...
extends Module {
        ...
        OffscreenESP.mc.theWorld.loadedEntityList.forEach(o -> {
            if (o instanceof EntityPlayer && this.isValid((EntityPlayer)o)) {
                ...
                if (pos != null && !this.isOnScreen(pos) && !RenderUtil.isInViewFrustrum((Entity)entity)) {
                    Color color = ColorUtil.getColor((Entity)entity, this.c.getValue().getRed(), this.c.getValue().getGreen(), this.c.getValue().getBlue(), (int)MathHelper.clamp_float((float)(255.0f - 255.0f / (float)this.fadeDistance.getValue().intValue() * OffscreenESP.mc.thePlayer.getDistanceToEntity((Entity)entity)), (float)100.0f, (float)255.0f), this.onlyFriends.getValue() == false);
                    ...
                    float yaw = this.getRotations((EntityLivingBase)entity) - OffscreenESP.mc.thePlayer.rotationYaw;
    ...
    private boolean isValid(EntityPlayer entity) {
        if (entity != OffscreenESP.mc.thePlayer && !entity.isInvisible()) {
    ...
    private float getRotations(EntityLivingBase ent) {
        double x = ent.posX - OffscreenESP.mc.thePlayer.posX;
        double z = ent.posZ - OffscreenESP.mc.thePlayer.posZ;
    ...
    private static class EntityListener {
        ...
        private void render() {
            ...
            for (Entity e : Util.mc.theWorld.loadedEntityList) {
