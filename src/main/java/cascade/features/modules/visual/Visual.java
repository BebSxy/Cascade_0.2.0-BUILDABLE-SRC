...
extends Module {
    ...
    public void onUpdate() {
        ...
        if (this.instantSwap.getValue().booleanValue() && (double)Visual.mc.entityRenderer.itemRenderer.prevEquippedProgressMainHand >= 0.9) {
            Visual.mc.entityRenderer.itemRenderer.equippedProgressMainHand = 1.0f;
            Visual.mc.entityRenderer.itemRenderer.itemStackMainHand = Visual.mc.thePlayer.getHeldItemMainhand();
        }
    ...
    public void renderShulkerToolTip(ItemStack stack, int x, int y, String name) {
        ...
        if (tagCompound != null && tagCompound.hasKey("BlockEntityTag", 10) && (blockEntityTag = tagCompound.getCompoundTag("BlockEntityTag")).hasKey("Items", 9)) {
            ...
            for (int i = 0; i < nonnulllist.size(); ++i) {
                ...
                Visual.mc.getRenderItem().zLevel = 501.0f;
                RenderUtil.itemRender.renderItemAndEffectIntoGUI(itemStack, iX, iY);
                RenderUtil.itemRender.renderItemOverlayIntoGUI(Visual.mc.fontRendererObj, itemStack, iX, iY, null);
                Visual.mc.getRenderItem().zLevel = 0.0f;
            }
            GlStateManager.disableLighting();
