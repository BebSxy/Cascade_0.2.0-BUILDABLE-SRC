...
extends Module {
    ...
    public void onPop(TotemPopEvent event) {
        if (PopChams.mc.theWorld.getEntityByID(event.getEntity().entityId) != null && this.isEnabled()) {
            if (this.antiSelf.getValue().booleanValue() && event.getEntity().entityId == PopChams.mc.thePlayer.getEntityId()) {
                return;
            }
            Entity entity = PopChams.mc.theWorld.getEntityByID(event.getEntity().entityId);
            if (entity instanceof EntityPlayer) {
                EntityPlayer player = (EntityPlayer)entity;
                this.fakeEntity = new EntityOtherPlayerMP((World)PopChams.mc.theWorld, player.getGameProfile());
    ...
    public void onDeath(int entityId) {
        Entity entity;
        if (this.onDeath.getValue().booleanValue() && PopChams.mc.theWorld.getEntityByID(entityId) != null && (entity = PopChams.mc.theWorld.getEntityByID(entityId)) instanceof EntityPlayer) {
            EntityPlayer player = (EntityPlayer)entity;
            this.fakeEntity = new EntityOtherPlayerMP((World)PopChams.mc.theWorld, player.getGameProfile());
            this.fakeEntity.copyLocationAndAnglesFrom((Entity)player);
            this.fakeEntity.rotationYawHead = player.rotationYawHead;
    ...
    public void handlePopESP(int entityId) {
        Entity entity;
        if (PopChams.mc.theWorld.getEntityByID(entityId) != null && (entity = PopChams.mc.theWorld.getEntityByID(entityId)) instanceof EntityPlayer) {
            EntityPlayer player = (EntityPlayer)entity;
            this.fakeEntity = new EntityOtherPlayerMP((World)PopChams.mc.theWorld, player.getGameProfile());
            this.fakeEntity.copyLocationAndAnglesFrom((Entity)player);
            this.fakeEntity.rotationYawHead = player.rotationYawHead;
