...
extends Module {
    ...
    void drawTileEntities() {
        ...
        EntityPlayerSP renderEntity = mc.getRenderViewEntity() == null ? ESP.mc.thePlayer : mc.getRenderViewEntity();
        try {
            ...
            for (TileEntity tileEntity : ESP.mc.theWorld.loadedTileEntityList) {
                if (!(tileEntity instanceof TileEntityChest) && !(tileEntity instanceof TileEntityEnderChest) || ESP.mc.thePlayer.getDistance((double)tileEntity.getPos().getX(), (double)tileEntity.getPos().getY(), (double)tileEntity.getPos().getZ()) > (double)this.storageRange.getValue().floatValue()) continue;
