...
extends Module {
    ...
    public void onPacketSend(PacketEvent.Send event) {
        ...
        if (event.getStage() == 0 && event.getPacket() instanceof CPacketUseEntity && (cPacketUseEntity = (CPacketUseEntity)event.getPacket()).getAction() == CPacketUseEntity.Action.ATTACK && cPacketUseEntity.getEntityFromWorld((World)AutoCrystal.mc.theWorld) instanceof EntityEnderCrystal) {
            ...
            if (AutoCrystal.mc.thePlayer.getHeldItemOffhand().getItem() != Items.END_CRYSTAL) {
                int crystalSlot;
                int n = crystalSlot = AutoCrystal.mc.thePlayer.getHeldItemMainhand().getItem() == Items.END_CRYSTAL ? AutoCrystal.mc.thePlayer.inventory.currentItem : -1;
                if (crystalSlot == -1) {
                    for (int l = 0; l < 9; ++l) {
                        if (AutoCrystal.mc.thePlayer.inventory.getStackInSlot(l).getItem() != Items.END_CRYSTAL) continue;
                        crystalSlot = l;
                        this.hotBarSlot = l;
                        break;
                    }
            ...
            if (this.pos != null && AutoCrystal.mc.thePlayer.onGround) {
                RayTraceResult result = AutoCrystal.mc.theWorld.rayTraceBlocks(new Vec3d(AutoCrystal.mc.thePlayer.posX, AutoCrystal.mc.thePlayer.posY + (double)AutoCrystal.mc.thePlayer.getEyeHeight(), AutoCrystal.mc.thePlayer.posZ), new Vec3d((double)this.pos.x + 0.5, (double)this.pos.y + 1.0, (double)this.pos.z + 0.5));
                EnumFacing f = result == null || result.sideHit == null ? EnumFacing.UP : result.sideHit;
                AutoCrystal.mc.thePlayer.sendQueue.addToSendQueue((Packet)new CPacketPlayerTryUseItemOnBlock(this.pos, f, AutoCrystal.mc.thePlayer.getHeldItemOffhand().getItem() == Items.END_CRYSTAL ? EnumHand.OFF_HAND : EnumHand.MAIN_HAND, (float)this.pos.x, (float)this.pos.y, (float)this.pos.z));
                AutoCrystal.mc.thePlayer.swingArm(EnumHand.MAIN_HAND);
            }
        }
    }
    ...
    public void onCrystal() {
        ...
        if (this.crystal != null && this.explode.getValue().booleanValue()) {
            ...
            if (this.breakTimer.passedMs(this.breakDelay.getValue().longValue())) {
                ...
                if (this.swingMode.getValue() == SwingMode.OffHand) {
                    AutoCrystal.mc.thePlayer.swingArm(EnumHand.OFF_HAND);
                }
                int oldSlot = AutoCrystal.mc.thePlayer.inventory.currentItem;
                int n = swordSlot = AutoCrystal.mc.thePlayer.getHeldItemMainhand().getItem() == Items.diamond_sword ? AutoCrystal.mc.thePlayer.inventory.currentItem : -1;
                if (swordSlot == -1) {
                    for (int i = 0; i < 9; ++i) {
        ...
        if (this.placeTimer.passedMs(this.placeDelay.getValue().longValue()) && this.place.getValue().booleanValue()) {
            ...
            if (this.swapType.getValue() == SwapMode.Normal && AutoCrystal.mc.thePlayer.getHeldItemMainhand().getItem() != Items.END_CRYSTAL && AutoCrystal.mc.thePlayer.getHeldItemOffhand().getItem() != Items.END_CRYSTAL) {
                AutoCrystal.mc.thePlayer.inventory.currentItem = this.hotBarSlot;
                return;
            }
            if (this.swapType.getValue() == SwapMode.Silent && AutoCrystal.mc.thePlayer.getHeldItemMainhand().getItem() != Items.END_CRYSTAL) {
                AutoCrystal.mc.thePlayer.inventory.currentItem = this.hotBarSlot;
            ...
            if (this.swapType.getValue() != SwapMode.Silent && AutoCrystal.mc.thePlayer.getHeldItemMainhand().getItem() != Items.END_CRYSTAL && AutoCrystal.mc.thePlayer.getHeldItemOffhand().getItem() != Items.END_CRYSTAL) {
                this.pos = null;
                this.calcPos = null;
                this.target = null;
                this.realTarget = null;
            ...
            if (!this.ignoreUseAmount.getValue().booleanValue()) {
                ...
                if (!this.exploded) {
                    RayTraceResult result2 = AutoCrystal.mc.theWorld.rayTraceBlocks(new Vec3d(AutoCrystal.mc.thePlayer.posX, AutoCrystal.mc.thePlayer.posY + (double)AutoCrystal.mc.thePlayer.getEyeHeight(), AutoCrystal.mc.thePlayer.posZ), new Vec3d((double)this.pos.x + 0.5, (double)this.pos.y + 1.0, (double)this.pos.z + 0.5));
                    EnumFacing f2 = result2 == null || result2.sideHit == null ? EnumFacing.UP : result2.sideHit;
                    AutoCrystal.mc.thePlayer.sendQueue.addToSendQueue((Packet)new CPacketPlayerTryUseItemOnBlock(this.pos, f2, AutoCrystal.mc.thePlayer.getHeldItemOffhand().getItem() == Items.END_CRYSTAL ? EnumHand.OFF_HAND : EnumHand.MAIN_HAND, (float)this.pos.x, (float)this.pos.y, (float)this.pos.z));
                    AutoCrystal.mc.thePlayer.swingArm(EnumHand.MAIN_HAND);
                    this.renderSpots.put(this.pos, this.c.getValue().getAlpha());
                } else {
                    this.exploded = false;
    ...
    private void doCalculations() {
        ...
        for (BlockPos blockPos : this.placePostions(this.placeRange.getValue().floatValue())) {
            ...
            double localDmg = this.calculateDamage((double)blockPos.getX() + 0.5, (double)blockPos.getY() + 1.0, (double)blockPos.getZ() + 0.5, (Entity)AutoCrystal.mc.thePlayer);
            this.armor = false;
            ...
            double selfDmg = (double)this.calculateDamage((double)blockPos.getX() + 0.5, (double)blockPos.getY() + 1.0, (double)blockPos.getZ() + 0.5, (Entity)AutoCrystal.mc.thePlayer) + 4.0;
    ...
    private boolean canPlaceCrystal(BlockPos blockPos, boolean specialEntityCheck) {
        ...
        if (!BlockUtil.rayTracePlaceCheck(blockPos) ? AutoCrystal.mc.thePlayer.getDistanceSq(blockPos) > MathUtil.square(this.placeWallRange.getValue().floatValue()) : AutoCrystal.mc.thePlayer.getDistanceSq(blockPos) > MathUtil.square(this.placeRange.getValue().floatValue())) {
            return false;
        }
        try {
            if (AutoCrystal.mc.theWorld.getBlockState(blockPos).getBlock() != Blocks.bedrock && AutoCrystal.mc.theWorld.getBlockState(blockPos).getBlock() != Blocks.obsidian) {
                return false;
            }
            if (AutoCrystal.mc.theWorld.getBlockState(boost).getBlock() != Blocks.air) {
                return false;
            }
            if (AutoCrystal.mc.theWorld.getBlockState(boost2).getBlock() != Blocks.air) {
                return false;
            }
    ...
    public static RayTraceResult rayTraceBlocks(Vec3d vec31, Vec3d vec32, boolean stopOnLiquid, boolean ignoreNoBox, boolean returnLastUncollidableBlock, boolean ignoreWebs) {
        ...
        if (!(Double.isNaN(vec32.xCoord) || Double.isNaN(vec32.yCoord) || Double.isNaN(vec32.zCoord))) {
            ...
            IBlockState state = AutoCrystal.mc.theWorld.getBlockState(pos);
            Block block = state.getBlock();
            if (!(ignoreNoBox && state.getCollisionBoundingBox((IBlockAccess)AutoCrystal.mc.theWorld, pos) == Block.NULL_AABB || !block.canCollideCheck(state, stopOnLiquid) || ignoreWebs && block instanceof BlockWeb || (raytraceresult = state.collisionRayTrace((World)AutoCrystal.mc.theWorld, pos, vec31, vec32)) == null)) {
            ...
            while (k1-- >= 0) {
                ...
                IBlockState state2 = AutoCrystal.mc.theWorld.getBlockState(pos);
                Block block2 = state2.getBlock();
                if (ignoreNoBox && state2.getMaterial() != Material.portal && state2.getCollisionBoundingBox((IBlockAccess)AutoCrystal.mc.theWorld, pos) == Block.NULL_AABB) continue;
                if (!(!block2.canCollideCheck(state2, stopOnLiquid) || ignoreWebs && block2 instanceof BlockWeb)) {
                    RayTraceResult raytraceresult3 = state2.collisionRayTrace((World)AutoCrystal.mc.theWorld, pos, vec31, vec32);
    ...
    public static boolean isOnGround(double x, double y, double z, Entity entity) {
        try {
            double d3 = y;
            List list1 = AutoCrystal.mc.theWorld.getCollisionBoxes(entity, entity.getEntityBoundingBox().addCoord(x, y, z));
            if (y != 0.0) {
                int l = list1.size();
                for (int k = 0; k < l; ++k) {
    ...
    public static EntityPlayer placeValue(double x, double y, double z, EntityPlayer entity) {
        List list1 = AutoCrystal.mc.theWorld.getCollisionBoxes((Entity)entity, entity.getEntityBoundingBox().addCoord(x, y, z));
        if (y != 0.0) {
            int l = list1.size();
            for (int k = 0; k < l; ++k) {
                y = ((AxisAlignedBB)list1.get(k)).calculateYOffset(entity.getEntityBoundingBox(), y);
    ...
    private float getDamageMultiplied(float damage) {
        int diff = AutoCrystal.mc.theWorld.getDifficulty().getDifficultyId();
