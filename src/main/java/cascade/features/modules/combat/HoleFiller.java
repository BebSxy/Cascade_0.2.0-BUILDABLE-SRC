...
extends Module {
    ...
    public void onPacketReceive(PacketEvent.Receive e) {
        ...
        if (this.predict.getValue().booleanValue() && e.getPacket() instanceof SPacketBlockChange) {
            ...
            if (p.blockState.getBlock() == Blocks.air && this.holes.contains(p.blockPosition) && this.predictBpt.getValue() <= this.packets) {
                mc.addScheduledTask(() -> this.doHoleFill());
                ++this.packets;
            }
        }
    ...
    private void doHoleFill() {
        ...
        Iterable blocks = BlockPos.getAllInBox((BlockPos)HoleFiller.mc.thePlayer.getPosition().add((double)(-this.range.getValue().floatValue()), (double)(-this.range.getValue().floatValue()), (double)(-this.range.getValue().floatValue())), (BlockPos)HoleFiller.mc.thePlayer.getPosition().add((double)this.range.getValue().floatValue(), (double)this.range.getValue().floatValue(), (double)this.range.getValue().floatValue()));
        for (BlockPos pos : blocks) {
            boolean solidNeighbours;
            if (HoleFiller.mc.theWorld.getBlockState(pos).getMaterial().blocksMovement() || HoleFiller.mc.theWorld.getBlockState(pos.add(0, 1, 0)).getMaterial().blocksMovement()) continue;
            boolean bl = HoleFiller.mc.theWorld.getBlockState(pos.add(1, 0, 0)).getBlock() == Blocks.bedrock | HoleFiller.mc.theWorld.getBlockState(pos.add(1, 0, 0)).getBlock() == Blocks.obsidian && HoleFiller.mc.theWorld.getBlockState(pos.add(0, 0, 1)).getBlock() == Blocks.bedrock | HoleFiller.mc.theWorld.getBlockState(pos.add(0, 0, 1)).getBlock() == Blocks.obsidian && HoleFiller.mc.theWorld.getBlockState(pos.add(-1, 0, 0)).getBlock() == Blocks.bedrock | HoleFiller.mc.theWorld.getBlockState(pos.add(-1, 0, 0)).getBlock() == Blocks.obsidian && HoleFiller.mc.theWorld.getBlockState(pos.add(0, 0, -1)).getBlock() == Blocks.bedrock | HoleFiller.mc.theWorld.getBlockState(pos.add(0, 0, -1)).getBlock() == Blocks.obsidian && HoleFiller.mc.theWorld.getBlockState(pos.add(0, 0, 0)).getMaterial() == Material.air && HoleFiller.mc.theWorld.getBlockState(pos.add(0, 1, 0)).getMaterial() == Material.air && HoleFiller.mc.theWorld.getBlockState(pos.add(0, 2, 0)).getMaterial() == Material.air ? true : (solidNeighbours = false);
