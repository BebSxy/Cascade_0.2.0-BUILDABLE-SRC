...
extends Module {
    ...
    public void onUpdate() {
        ...
        if (this.normal.getValue().booleanValue() && BowSpam.mc.thePlayer.getHeldItemMainhand().getItem() instanceof ItemBow && BowSpam.mc.thePlayer.isHandActive() && BowSpam.mc.thePlayer.getItemInUseMaxCount() >= this.ticks.getValue()) {
            mc.getNetHandler().addToSendQueue((Packet)new CPacketPlayerDigging(CPacketPlayerDigging.Action.RELEASE_USE_ITEM, BlockPos.ORIGIN, BowSpam.mc.thePlayer.getHorizontalFacing()));
            mc.getNetHandler().addToSendQueue((Packet)new CPacketPlayerTryUseItem(BowSpam.mc.thePlayer.getActiveHand()));
            BowSpam.mc.thePlayer.stopActiveHand();
        }
    }
    ...
    public void onUpdateWalkingPlayer(UpdateWalkingPlayerEvent e) {
        ...
        if (this.rape.getValue().booleanValue() && BowSpam.mc.thePlayer.onGround && stack != null && !BowSpam.mc.thePlayer.getActiveItemStack().isEmpty() && BowSpam.mc.thePlayer.getItemInUseCount() > 0) {
            Cascade.timerManager.set(this.timer.getValue().floatValue());
            if ((float)(stack.getMaxItemUseDuration() - BowSpam.mc.thePlayer.getItemInUseCount()) > (float)this.ticks.getValue().intValue() * this.timer.getValue().floatValue()) {
                BowSpam.mc.playerController.onStoppedUsingItem((EntityPlayer)BowSpam.mc.thePlayer);
            }
        }
        if (this.bowBomb.getValue().booleanValue() && stack != null && !BowSpam.mc.thePlayer.getActiveItemStack().isEmpty() && stack.getMaxItemUseDuration() - BowSpam.mc.thePlayer.getItemInUseCount() >= this.ticks.getValue()) {
    ...
    private ItemStack getStack() {
        ItemStack mainHand = BowSpam.mc.thePlayer.getHeldItemMainhand();
        if (mainHand.getItem() instanceof ItemBow) {
            return mainHand;
        }
        ItemStack offHand = BowSpam.mc.thePlayer.getHeldItemOffhand();
