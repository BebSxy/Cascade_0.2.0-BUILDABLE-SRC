...
extends Module {
    ...
    private void SwitchOffHandIfNeed() {
        ...
        if (Offhand.mc.thePlayer.getHeldItemOffhand().getItem() != i) {
            int l_Slot;
            ...
            if ((l_Slot = this.GetItemSlot(i)) != -1) {
                Offhand.mc.playerController.windowClick(Offhand.mc.thePlayer.inventoryContainer.windowId, l_Slot, 0, ClickType.PICKUP, (EntityPlayer)Offhand.mc.thePlayer);
                Offhand.mc.playerController.windowClick(Offhand.mc.thePlayer.inventoryContainer.windowId, 45, 0, ClickType.PICKUP, (EntityPlayer)Offhand.mc.thePlayer);
                Offhand.mc.playerController.windowClick(Offhand.mc.thePlayer.inventoryContainer.windowId, l_Slot, 0, ClickType.PICKUP, (EntityPlayer)Offhand.mc.thePlayer);
    ...
    public Item getItemType() {
        switch (this.mode.getValue()) {
            ...
            case Crystal: {
                ...
                if (!(!(Offhand.mc.thePlayer.motionY < 0.0) || !(Offhand.mc.thePlayer.fallDistance > this.fallDistance.getValue().floatValue()) || Offhand.mc.thePlayer.isElytraFlying() || !this.forceGap.getValue().booleanValue() || this.gapSword.getValue().booleanValue() && InventoryUtil.heldItem(Items.diamond_sword, InventoryUtil.Hand.Main) && Offhand.mc.gameSettings.keyBindUseItem.getIsKeyPressed())) {
                    return Items.TOTEM_OF_UNDYING;
                }
                if (this.totemElytra.getValue().booleanValue() && Offhand.mc.thePlayer.isElytraFlying() && this.forceGap.getValue().booleanValue() && (!this.gapSword.getValue().booleanValue() || !InventoryUtil.heldItem(Items.diamond_sword, InventoryUtil.Hand.Main) || !Offhand.mc.gameSettings.keyBindUseItem.getIsKeyPressed())) {
                    return Items.TOTEM_OF_UNDYING;
                }
                if (this.gapSword.getValue().booleanValue() && InventoryUtil.heldItem(Items.diamond_sword, InventoryUtil.Hand.Main) && Offhand.mc.gameSettings.keyBindUseItem.getIsKeyPressed()) {
                    return Items.golden_apple;
            ...
            case GApple: {
                ...
                if (Offhand.mc.thePlayer.motionY < 0.0 && Offhand.mc.thePlayer.fallDistance > this.fallDistance.getValue().floatValue() && !Offhand.mc.thePlayer.isElytraFlying()) {
                    return Items.TOTEM_OF_UNDYING;
                }
                if (this.totemElytra.getValue().booleanValue() && Offhand.mc.thePlayer.isElytraFlying()) {
                    return Items.TOTEM_OF_UNDYING;
                }
                return Items.golden_apple;
            }
    ...
    public int GetItemSlot(Item item) {
        ...
        for (int i = 0; i < Offhand.mc.thePlayer.inventoryContainer.getInventory().size(); ++i) {
            ItemStack s;
            if (i == 0 || i == 5 || i == 6 || i == 7 || i == 8 || (s = (ItemStack)Offhand.mc.thePlayer.inventoryContainer.getInventory().get(i)).isEmpty() || s.getItem() != item) continue;
            return i;
        }
        return -1;
    }
