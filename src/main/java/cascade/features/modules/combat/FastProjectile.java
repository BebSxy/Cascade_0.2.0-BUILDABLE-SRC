...
extends Module {
    ...
    public void onUpdateWalkingPlayer(UpdateWalkingPlayerEvent event) {
        ...
        if (event.getStage() != 0) {
            if (FastProjectile.mc.thePlayer.getActiveItemStack().getItem() != Items.bow && this.mode.getValue() == Mode.Bow || FastProjectile.mc.thePlayer.getActiveItemStack().getItem() != Items.egg && this.mode.getValue() == Mode.Egg || FastProjectile.mc.thePlayer.getActiveItemStack().getItem() != Items.snowball && this.mode.getValue() == Mode.Snowball) {
                this.cancelling = false;
                this.packetsSent = 0;
            } else if (FastProjectile.mc.thePlayer.getActiveItemStack().getItem() == Items.bow && FastProjectile.mc.thePlayer.isHandActive() && this.cancelling) {
                ++this.packetsSent;
                if (this.packetsSent <= this.runs.getValue() * 2 || !this.always.getValue().booleanValue()) {
                    // empty if block
                }
    ...
    public boolean heldIemCheck() {
        if (FastProjectile.mc.thePlayer.getActiveItemStack().getItem() == Items.bow && this.mode.getValue() == Mode.Bow) {
            return true;
        }
        if (FastProjectile.mc.thePlayer.getActiveItemStack().getItem() == Items.egg && this.mode.getValue() == Mode.Egg) {
            return true;
        }
        return FastProjectile.mc.thePlayer.getActiveItemStack().getItem() == Items.snowball && this.mode.getValue() == Mode.Snowball;
    }
    ...
    public void onRightClickItem(RightClickItemEvent e) {
        ...
        if (FastProjectile.mc.thePlayer.getHeldItem(e.getHand()).getItem() == Items.bow) {
    ...
    protected void onPacketSend(PacketEvent.Send e) {
        ...
        if (e.getPacket() instanceof CPacketPlayerDigging && ((CPacketPlayerDigging)e.getPacket()).getAction() == CPacketPlayerDigging.Action.RELEASE_USE_ITEM && this.heldIemCheck()) {
            this.cancelling = false;
            if (this.packetsSent >= this.runs.getValue() * 2 || this.always.getValue().booleanValue()) {
