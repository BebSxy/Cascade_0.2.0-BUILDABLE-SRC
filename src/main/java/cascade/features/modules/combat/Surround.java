...
extends Module {
    ...
    public void onPacketReceive(PacketEvent.Receive e) {
        ...
        if (this.predict.getValue().booleanValue() && e.getPacket() instanceof SPacketBlockChange) {
            ...
            if (p.blockState.getBlock() == Blocks.air && Surround.mc.thePlayer.getDistance((double)p.blockPosition.x, (double)p.blockPosition.y, (double)p.blockPosition.z) < 1.75 && this.predictBpt.getValue() <= this.packets) {
    ...
    void doFeetPlace() {
        ...
        if (PlayerUtil.isChestBelow()) {
            offsets.add(Surround.mc.thePlayer.getPositionVector().addVector(1.0, 0.0, 0.0));
            offsets.add(Surround.mc.thePlayer.getPositionVector().addVector(-1.0, 0.0, 0.0));
            offsets.add(Surround.mc.thePlayer.getPositionVector().addVector(0.0, 0.0, 1.0));
            offsets.add(Surround.mc.thePlayer.getPositionVector().addVector(0.0, 0.0, -1.0));
            offsets.add(Surround.mc.thePlayer.getPositionVector().addVector(1.0, 1.0, 0.0));
            offsets.add(Surround.mc.thePlayer.getPositionVector().addVector(-1.0, 1.0, 0.0));
            offsets.add(Surround.mc.thePlayer.getPositionVector().addVector(0.0, 1.0, 1.0));
            offsets.add(Surround.mc.thePlayer.getPositionVector().addVector(0.0, 1.0, -1.0));
        } else {
            offsets.add(Surround.mc.thePlayer.getPositionVector().addVector(0.0, -1.0, 0.0));
            offsets.add(Surround.mc.thePlayer.getPositionVector().addVector(1.0, -1.0, 0.0));
            offsets.add(Surround.mc.thePlayer.getPositionVector().addVector(-1.0, -1.0, 0.0));
            offsets.add(Surround.mc.thePlayer.getPositionVector().addVector(0.0, -1.0, 1.0));
            offsets.add(Surround.mc.thePlayer.getPositionVector().addVector(0.0, -1.0, -1.0));
        ...
        for (Vec3d vec3d : offsets) {
            ...
            if (!Surround.mc.theWorld.getBlockState(pos).getMaterial().isReplaceable()) continue;
            ...
            if (this.extend.getValue().booleanValue()) {
                try {
                    for (Entity en : Surround.mc.theWorld.loadedEntityList) {
                        if (!(en instanceof EntityPlayer) || !Surround.mc.thePlayer.boundingBox.intersectsWith(en.getEntityBoundingBox())) continue;
                        offsets.add(en.getPositionVector().addVector(1.0, 0.0, 0.0));
                        offsets.add(en.getPositionVector().addVector(0.0, 0.0, 1.0));
                        offsets.add(en.getPositionVector().addVector(-1.0, 0.0, 0.0));
                        offsets.add(en.getPositionVector().addVector(0.0, 0.0, -1.0));
                    }
                }
