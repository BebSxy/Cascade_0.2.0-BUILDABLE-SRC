...
extends Module {
    ...
    public void onPacketSend(PacketEvent.Send event) {
        ...
        if (event.getStage() == 0 && event.getPacket() instanceof CPacketUseEntity && (cPacketUseEntity = (CPacketUseEntity)event.getPacket()).getAction() == CPacketUseEntity.Action.ATTACK && cPacketUseEntity.getEntityFromWorld((World)CrystalAura.mc.theWorld) instanceof EntityEnderCrystal) {
            ...
            if (CrystalAura.mc.thePlayer.getHeldItemOffhand().getItem() != Items.END_CRYSTAL) {
                int crystalSlot;
                int n = crystalSlot = CrystalAura.mc.thePlayer.getHeldItemMainhand().getItem() == Items.END_CRYSTAL ? CrystalAura.mc.thePlayer.inventory.currentItem : -1;
                if (crystalSlot == -1) {
                    for (int l = 0; l < 9; ++l) {
                        if (CrystalAura.mc.thePlayer.inventory.getStackInSlot(l).getItem() != Items.END_CRYSTAL) continue;
                        crystalSlot = l;
                        this.hotBarSlot = l;
                        break;
                    }
            ...
            if (this.pos != null && CrystalAura.mc.thePlayer.onGround) {
                RayTraceResult result = CrystalAura.mc.theWorld.rayTraceBlocks(new Vec3d(CrystalAura.mc.thePlayer.posX, CrystalAura.mc.thePlayer.posY + (double)CrystalAura.mc.thePlayer.getEyeHeight(), CrystalAura.mc.thePlayer.posZ), new Vec3d((double)this.pos.x + 0.5, (double)this.pos.y + 1.0, (double)this.pos.z + 0.5));
                EnumFacing f = result == null || result.sideHit == null ? EnumFacing.UP : result.sideHit;
                mc.getNetHandler().addToSendQueue((Packet)new CPacketPlayerTryUseItemOnBlock(this.pos, f, CrystalAura.mc.thePlayer.getHeldItemOffhand().getItem() == Items.END_CRYSTAL ? EnumHand.OFF_HAND : EnumHand.MAIN_HAND, (float)this.pos.x, (float)this.pos.y, (float)this.pos.z));
                CrystalAura.mc.thePlayer.swingArm(EnumHand.MAIN_HAND);
            }
        }
    }
    ...
    public void onCrystal() {
        ...
        if (this.crystal != null && this.explode.getValue().booleanValue()) {
            ...
            if (this.breakTimer.passedMs(this.breakDelay.getValue().longValue())) {
                ...
                int n = swordSlot = CrystalAura.mc.thePlayer.getHeldItemMainhand().getItem() == Items.diamond_sword ? CrystalAura.mc.thePlayer.inventory.currentItem : -1;
                if (swordSlot == -1) {
                    for (int i = 0; i < 9; ++i) {
                ...
                if (this.swingMode.getValue() == SwingMode.OffHand) {
                    CrystalAura.mc.thePlayer.swingArm(EnumHand.OFF_HAND);
                }
        ...
        if (this.placeTimer.passedMs(this.placeDelay.getValue().longValue()) && this.place.getValue().booleanValue()) {
            ...
            int crystalSlot = InventoryUtil.getItemSlot(Items.END_CRYSTAL);
            int oldSlot = CrystalAura.mc.thePlayer.inventory.currentItem;
            if (this.swap.getValue() == Swap.Off && !InventoryUtil.heldItem(Items.END_CRYSTAL, InventoryUtil.Hand.Both)) {
                this.pos = null;
                this.calcPos = null;
                this.target = null;
                this.realTarget = null;
            ...
            if (!this.ignoreUseAmount.getValue().booleanValue()) {
                ...
                if (this.crystalCount < crystalLimit && this.pos != null) {
                    if (!this.exploded) {
                        EnumFacing f;
                        RayTraceResult result = CrystalAura.mc.theWorld.rayTraceBlocks(new Vec3d(CrystalAura.mc.thePlayer.posX, CrystalAura.mc.thePlayer.posY + (double)CrystalAura.mc.thePlayer.getEyeHeight(), CrystalAura.mc.thePlayer.posZ), new Vec3d((double)this.pos.x + 0.5, (double)this.pos.y + 1.0, (double)this.pos.z + 0.5));
                        EnumFacing enumFacing = f = result == null || result.sideHit == null ? EnumFacing.UP : result.sideHit;
                        if (this.swap.getValue() == Swap.Silent && !InventoryUtil.heldItem(Items.END_CRYSTAL, InventoryUtil.Hand.Both) && crystalSlot != -1) {
                            mc.getNetHandler().addToSendQueue((Packet)new CPacketHeldItemChange(crystalSlot));
                        }
                        mc.getNetHandler().addToSendQueue((Packet)new CPacketPlayerTryUseItemOnBlock(this.pos, f, CrystalAura.mc.thePlayer.getHeldItemOffhand().getItem() == Items.END_CRYSTAL ? EnumHand.OFF_HAND : EnumHand.MAIN_HAND, (float)this.pos.x, (float)this.pos.y, (float)this.pos.z));
                        if (this.swap.getValue() == Swap.Silent && !InventoryUtil.heldItem(Items.END_CRYSTAL, InventoryUtil.Hand.Both)) {
                ...
                if (!this.exploded) {
                    EnumFacing f2;
                    RayTraceResult result2 = CrystalAura.mc.theWorld.rayTraceBlocks(new Vec3d(CrystalAura.mc.thePlayer.posX, CrystalAura.mc.thePlayer.posY + (double)CrystalAura.mc.thePlayer.getEyeHeight(), CrystalAura.mc.thePlayer.posZ), new Vec3d((double)this.pos.x + 0.5, (double)this.pos.y + 1.0, (double)this.pos.z + 0.5));
                    EnumFacing enumFacing = f2 = result2 == null || result2.sideHit == null ? EnumFacing.UP : result2.sideHit;
                    if (this.swap.getValue() == Swap.Silent && !InventoryUtil.heldItem(Items.END_CRYSTAL, InventoryUtil.Hand.Both) && crystalSlot != -1) {
                        mc.getNetHandler().addToSendQueue((Packet)new CPacketHeldItemChange(crystalSlot));
                    }
                    mc.getNetHandler().addToSendQueue((Packet)new CPacketPlayerTryUseItemOnBlock(this.pos, f2, CrystalAura.mc.thePlayer.getHeldItemOffhand().getItem() == Items.END_CRYSTAL ? EnumHand.OFF_HAND : EnumHand.MAIN_HAND, (float)this.pos.x, (float)this.pos.y, (float)this.pos.z));
                    CrystalAura.mc.thePlayer.swingArm(EnumHand.MAIN_HAND);
                    if (this.swap.getValue() == Swap.Silent && !InventoryUtil.heldItem(Items.END_CRYSTAL, InventoryUtil.Hand.Both)) {
                        mc.getNetHandler().addToSendQueue((Packet)new CPacketHeldItemChange(oldSlot));
                    }
    ...
    void doCalculations() {
        ...
        for (BlockPos blockPos : this.placePostions(this.placeRange.getValue().floatValue())) {
            ...
            double localDmg = this.calculateDamage((double)blockPos.getX() + 0.5, (double)blockPos.getY() + 1.0, (double)blockPos.getZ() + 0.5, (Entity)CrystalAura.mc.thePlayer);
            this.armor = false;
            ...
            double selfDmg = (double)this.calculateDamage((double)blockPos.getX() + 0.5, (double)blockPos.getY() + 1.0, (double)blockPos.getZ() + 0.5, (Entity)CrystalAura.mc.thePlayer) + 4.0;
    ...
    public void onPacketReceive(PacketEvent.Receive e) {
            ...
            try {
                if (sPacketSoundEffect.getCategory() == SoundCategory.BLOCKS && sPacketSoundEffect.getSound() == SoundEvents.ENTITY_GENERIC_EXPLODE) {
                    for (Entity en : CrystalAura.mc.theWorld.loadedEntityList) {
                        if (!(en instanceof EntityEnderCrystal) || !(en.getDistance(sPacketSoundEffect.getX(), sPacketSoundEffect.getY(), sPacketSoundEffect.getZ()) <= (double)this.breakRange.getValue().floatValue())) continue;
        ...
        if (e.getPacket() instanceof SPacketExplosion) {
            try {
                ...
                for (Entity en : CrystalAura.mc.theWorld.loadedEntityList) {
                    if (!(en instanceof EntityEnderCrystal) || !(en.getDistance(sPacketExplosion.getX(), sPacketExplosion.getY(), sPacketExplosion.getZ()) <= (double)this.breakRange.getValue().floatValue())) continue;
        ...
        if (e.getPacket() instanceof SPacketSoundEffect && this.chorusPredict.getValue().booleanValue() && ((sPacketSoundEffect = (SPacketSoundEffect)e.getPacket()).getSound() == SoundEvents.ITEM_CHORUS_FRUIT_TELEPORT || sPacketSoundEffect.getSound() == SoundEvents.ENTITY_ENDERMEN_TELEPORT) && CrystalAura.mc.thePlayer.getDistance(sPacketSoundEffect.getX(), sPacketSoundEffect.getY(), sPacketSoundEffect.getZ()) <= (double)this.targetRange.getValue().floatValue()) {
    ...
    boolean isPredicting(SPacketSpawnObject packet) {
        try {
            ...
            return CrystalAura.mc.thePlayer.getDistance(packet.getX(), packet.getY(), packet.getZ()) <= (double)this.breakRange.getValue().floatValue() && (BlockUtil.rayTracePlaceCheck(packPos) || CrystalAura.mc.thePlayer.getDistance(packet.getX(), packet.getY(), packet.getZ()) <= (double)this.breakWallRange.getValue().floatValue()) && (this.ignoreUseAmount.getValue() != false || !this.target.isDead) && this.target.getHealth() + this.target.getAbsorptionAmount() > 0.0f;
        }
        catch (Exception e) {
            return false;
        }
    ...
    boolean canPlaceCrystal(BlockPos blockPos, boolean specialEntityCheck) {
        ...
        if (!BlockUtil.rayTracePlaceCheck(blockPos) ? CrystalAura.mc.thePlayer.getDistanceSq(blockPos) > MathUtil.square(this.placeWallRange.getValue().floatValue()) : CrystalAura.mc.thePlayer.getDistanceSq(blockPos) > MathUtil.square(this.placeRange.getValue().floatValue())) {
            return false;
        }
        try {
            if (this.holePlacement.getValue().booleanValue()) {
                if (CrystalAura.mc.theWorld.getBlockState(blockPos).getBlock() != Blocks.bedrock && CrystalAura.mc.theWorld.getBlockState(blockPos).getBlock() != Blocks.obsidian) {
                    return false;
                }
                if (CrystalAura.mc.theWorld.getBlockState(boost).getBlock() != Blocks.air) {
                    return false;
                }
                if (CrystalAura.mc.theWorld.getBlockState(boost2).getBlock() != Blocks.air) {
                    return false;
                }
    ...
    float calculateDamage(double posX, double posY, double posZ, Entity entity) {
        ...
        try {
            blockDensity = MathUtil.getBlockDensity(vec3d, entity.getEntityBoundingBox());
        }
        catch (Exception ex) {
            Cascade.LOGGER.info("Caught an exception from CrystalAura");
            ex.printStackTrace();
    ...
    private float getDamageMultiplied(float damage) {
        int diff = CrystalAura.mc.theWorld.getDifficulty().getDifficultyId();
