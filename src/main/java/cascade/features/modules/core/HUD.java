...
extends Module {
    ...
    public void onRender2D(Render2DEvent event) {
        ...
        if (this.pvpInfo.getValue().booleanValue()) {
            ...
            int totems = HUD.mc.thePlayer.inventory.mainInventory.stream().filter(itemStack -> itemStack.getItem() == Items.TOTEM_OF_UNDYING).mapToInt(ItemStack::getCount).sum() + (InventoryUtil.heldItem(Items.TOTEM_OF_UNDYING, InventoryUtil.Hand.Off) ? 1 : 0);
        ...
        boolean inHell = HUD.mc.theWorld.getBiome(HUD.mc.thePlayer.getPosition()).getBiomeName() == "Hell";
    ...
    void renderTotem() {
        int width = this.renderer.scaledWidth;
        int height = this.renderer.scaledHeight;
        int totems = HUD.mc.thePlayer.inventory.mainInventory.stream().filter(itemStack -> itemStack.getItem() == Items.TOTEM_OF_UNDYING).mapToInt(ItemStack::getCount).sum();
        if (HUD.mc.thePlayer.getHeldItemOffhand().getItem() == Items.TOTEM_OF_UNDYING) {
            totems += HUD.mc.thePlayer.getHeldItemOffhand().getCount();
        }
        if (totems > 0) {
            GlStateManager.enableTexture2D();
            int i = width / 2;
    ...
    private double calcSpeed(EntityPlayerSP player) {
        double tps = 1000.0 / (double)HUD.mc.timer.tickLength;
