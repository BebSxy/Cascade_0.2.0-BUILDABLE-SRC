...
extends Feature {
    ...
    public List<BlockPos> getSortedHoles() {
        this.holes.sort(Comparator.comparingDouble(hole -> HoleManager.mc.thePlayer.getDistanceSq(hole)));
    ...
    public List<BlockPos> calcHoles() {
        ...
        for (BlockPos pos : positions) {
            if (!HoleManager.mc.theWorld.getBlockState(pos).getBlock().equals(Blocks.air) || !HoleManager.mc.theWorld.getBlockState(pos.add(0, 1, 0)).getBlock().equals(Blocks.air) || !HoleManager.mc.theWorld.getBlockState(pos.add(0, 2, 0)).getBlock().equals(Blocks.air)) continue;
            boolean isSafe = true;
            boolean midSafe = true;
            for (BlockPos offset : surroundOffset) {
                Block block = HoleManager.mc.theWorld.getBlockState(pos.add((Vec3i)offset)).getBlock();
