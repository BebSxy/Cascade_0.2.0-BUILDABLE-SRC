...
extends Feature {
    ...
    public void onUpdate(LivingEvent.LivingUpdateEvent event) {
        if (!EventManager.fullNullCheck() && event.getEntity().getEntityWorld().isRemote && event.getEntityLiving() == EventManager.mc.thePlayer) {
            Cascade.inventoryManager.update();
            Cascade.moduleManager.onUpdate();
            if (HUD.getInstance().ordering.getValue() == HUD.Ordering.Length) {
                Cascade.moduleManager.sortModules(true);
            } else {
                Cascade.moduleManager.sortModulesABC();
            }
            if (HUD.getInstance().ordering.getValue() == HUD.Ordering.Length) {
                Cascade.moduleManager.sortModules(true);
            } else {
    ...
    public void onTick(TickEvent.ClientTickEvent e) {
        ...
        for (EntityPlayer player : EventManager.mc.theWorld.playerEntities) {
    ...
    public void onUpdateWalkingPlayer(UpdateWalkingPlayerEvent event) {
        ...
        if (event.getStage() == 0) {
            ...
            Cascade.rotationManager.updateRotations();
            Cascade.positionManager.updatePosition();
        }
        if (event.getStage() == 1) {
            Cascade.rotationManager.restoreRotations();
    ...
    public void onPacketReceive(PacketEvent.Receive event) {
        ...
        if (event.getPacket() instanceof SPacketEntityStatus && (packet = (SPacketEntityStatus)event.getPacket()).getOpCode() == 35 && packet.getEntity((World)EventManager.mc.theWorld) instanceof EntityPlayer) {
            EntityPlayer player = (EntityPlayer)packet.getEntity((World)EventManager.mc.theWorld);
    ...
    private static /* synthetic */ void lambda$onPacketReceive$1(SPacketPlayerListItem packet, SPacketPlayerListItem.AddPlayerData data) {
        ...
        switch (packet.getAction()) {
            ...
            case REMOVE_PLAYER: {
                EntityPlayer entity = EventManager.mc.theWorld.getPlayerEntityByUUID(id);
                if (entity != null) {
                    String logoutName = entity.getCommandSenderName();
